...
@section('header_subtitle', 'Daftarkan akun baru dan tentukan hak akses peran pengguna')
...
<div class="card" style="max-width: 700px; margin: 0 auto; position: relative; overflow: hidden;">
    ...
    <div class="section-header" style="margin-bottom: 24px; border-bottom: 1px solid var(--glass-border); padding-bottom: 16px;">
        <h3 class="section-title">
            <i class="bi bi-person-plus-fill text-accent" style="color: var(--color-accent);"></i>
    ...
    <form action="{{ route('users.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <div style="position: relative;">
                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="Contoh: Budi Santoso" value="{{ old('name') }}" required style="padding-left: 45px;">
        ...
        <div class="form-group">
            ...
            <div style="position: relative;">
                <input type="email" name="email" id="email" class="form-control @error('email') is-invalid @enderror" placeholder="nama@detpak.co.id" value="{{ old('email') }}" required style="padding-left: 45px;">
        ...
        <!-- Password -->
        <div class="form-group">
            <label for="password">Kata Sandi <span class="text-danger" style="color: var(--color-danger);">*</span></label>
            <div style="position: relative;">
                <input type="password" name="password" id="password" class="form-control @error('password') is-invalid @enderror" placeholder="Minimal 6 karakter..." required style="padding-left: 45px;">
        ...
        <div class="form-group">
            ...
            <div class="role-selector" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                <label style="display: flex; align-items: center; gap: 12px; padding: 16px; background: rgba(255, 255, 255, 0.02); border: 1px solid var(--glass-border); border-radius: var(--radius-md); cursor: pointer; transition: var(--transition-smooth);" class="role-option">
                    <input type="radio" name="role" value="user" {{ old('role', 'user') == 'user' ? 'checked' : '' }} style="accent-color: var(--color-primary);">
                    <div>
                ...
                <label style="display: flex; align-items: center; gap: 12px; padding: 16px; background: rgba(255, 255, 255, 0.02); border: 1px solid var(--glass-border); border-radius: var(--radius-md); cursor: pointer; transition: var(--transition-smooth);" class="role-option">
                    <input type="radio" name="role" value="admin" {{ old('role') == 'admin' ? 'checked' : '' }} style="accent-color: var(--color-primary);">
                    <div>
        ...
        <div class="form-actions">
            ...
            <button type="submit" class="btn btn-primary">
                <i class="bi bi-save-fill"></i> Simpan User Baru
            </button>
        </div>
    </form>
</div>
